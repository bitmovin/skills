...
the VOD encoding using the embedded `start` block. This script is a thin
wrapper that submits the YAML and writes a state file the rest of the
...
def _format_api_error(exc: Exception) -> str:
    ...
    to the user's terminal). Surface only short message + developerMessage
    + errorCode.
    """
...
def _extract_state(template_yaml: str, encoding_id: str) -> dict:
    ...
    start = (main.get("start") or {}).get("properties", {}) or {}
    ...
    manifest_files: dict[str, list[dict]] = {}
    for kind, manifests_or_default in (body.get("manifests") or {}).items():
        manifest_files[kind] = []
        # Manifests can be authored either as `<kind>.<name>` (custom) or
        # `<kind>.defaultapi.<name>` (default-manifest shortcut). Treat both
        # the same way for state purposes.
        items = (manifests_or_default or {}).get("defaultapi") or manifests_or_default or {}
        for mid, mdef in items.items():
            props = (mdef or {}).get("properties") or {}
            outputs = props.get("outputs") or [{}]
            manifest_files[kind].append(
                {
                    "templateId": mid,
                    "manifestName": props.get("manifestName"),
                    "outputId": outputs[0].get("outputId") if outputs else None,
                    "outputPath": outputs[0].get("outputPath") if outputs else None,
                }
            )
    ...
    return {
        ...
        "encodingMode": start.get("encodingMode"),
        "perTitle": "perTitle" in start,
...
def main(argv: list[str]) -> int:
    ...
    _verify_template_applied(api, encoding_id, state, template_path)
...
def _load_template_schema() -> dict | None:
    ...
    ``$BITMOVIN_VOD_SKILL_CACHE/template-schema-v<N>.json`` (default
    ``~/.cache/bitmovin/bitmovin-encoding-vod/``). On fetch failure we fall
    ...
    cache_root = pathlib.Path(
        os.environ.get(
            "BITMOVIN_VOD_SKILL_CACHE",
            pathlib.Path.home() / ".cache" / "bitmovin" / "bitmovin-encoding-vod",
        )
    )
...
def _verify_template_applied(
    ...
    *,
    ...
    certainly silently dropped (e.g. an unknown top-level key) and we fail
    loudly so the user sees it here rather than minutes into
    `wait_for_finished.py`.
    """
    ...
    sys.exit(
        ...
        "doesn't recognize.\n"
        f"  Inspect the rendered template at {template_path} and compare its "
        f"top-level structure against the working samples shipped with the "
        f"skill.{diag_block}"
    )
