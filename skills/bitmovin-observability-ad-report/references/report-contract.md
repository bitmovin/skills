...
| `cohort`          | object \| null                | The cohort filter applied to every pull (advertiser, ad system, country, ad type, etc.), or null. When the report is scoped to one ad type, record it here (e.g. `{adType: "ssai"}`) — see `references/ssai-csai-metrics.md`. |
...
| `errorPercentage`    | number [0,1]    | `ad_error_percentage` (avg). Null when no impressions.                  |
...
| `byAbandonment`  | `ParetoResult`   | Advertisers ranked by `(starts - completions)` — highest abandonment counts.    |
| `adSystems`      | `ParetoResult` \| null | Top ad systems by completions. Null when only one ad system was observed. |
| `tail`           | `{count: number, share: number}` | Roll-up of all entries below the gate threshold.                     |
