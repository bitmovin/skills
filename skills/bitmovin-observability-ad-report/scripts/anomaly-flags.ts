...
export function anomalyFlags(input: AnomalyFlagsInput): AnomalyFlagsResult {
  ...
  // ── Overall ad error percentage ───────────────────────────────────────────
  if (input.errors?.errorPercentage != null) {
    const observed = input.errors.errorPercentage;
    if (observed > thresholds.errorPercentageMax) {
      flags.push({
        id: "ad-error-percentage-high",
        severity: "high",
        message: `Ad error percentage is ${pct(observed)} (threshold ${pct(thresholds.errorPercentageMax)})`,
        detail: {
          observed,
          threshold: thresholds.errorPercentageMax,
        },
      });
    }
  }
