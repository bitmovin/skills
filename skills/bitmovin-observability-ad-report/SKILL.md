...
### Splitting SSAI vs CSAI

When the user explicitly says **SSAI / server-side** or **CSAI / client-side**,
apply the ad-type discriminator to **every** pull and report which side you
queried. **SSAI = 2, CSAI = 1.** Which attribute you filter on is chosen by the
metric's dataset, **not** the user's wording:

- An **`ad_*` metric** (ads dataset) → filter **`ad_type`**: `2` = SSAI,
  `1` = CSAI. This is the route for the funnel, quartiles, completions,
  startup time, and ad errors.
- A **non-`ad_` metric** (playback dataset, e.g. `rebuffer_percentage`,
  `samples`) → filter **`ad`**: `2` = SSAI, `1` = CSAI. `ad` is sample-level —
  pair it only with `samples`-based metrics, never `plays` / `unique_users` /
  `video_startuptime` (those return 0 on ad samples), and it is often sparse for
  SSAI.

The catalogs are disjoint: `ad_type` is rejected on a playback metric and `ad`
is rejected on an `ad_*` metric. Several quantities (ad error %, ad error
sessions / top codes) can be drawn from **either** dataset with different
denominators. For the per-metric source, aggregation, filter, and the
documented-but-unavailable metrics (failed beacon URLs, ad breaks abandoned, …),
read **`references/ssai-csai-metrics.md`** before constructing the queries —
don't guess the mapping, and don't mistake `is_linear` or `ad_module` for the
ad-type discriminator.
...
4. Capture any human-supplied scope (filter to a specific advertiser, ad
   ...
   pull. See HITL — context is enrichment, never required. **If the user names
   an ad type (SSAI / CSAI), treat it as a cohort filter on every pull** — filter
   `ad_type` on `ad_*` metrics and `ad` on playback metrics (`2` = SSAI,
   `1` = CSAI), per `references/ssai-csai-metrics.md`.
...
| `ad-error-percentage-high`    | > 5%          | High overall ad error rate                                      |
...
- **Sample-size at advertiser level.** Tiny advertisers swing wildly; gate
  every named advertiser through `sample-gate` (default minN = 1000
  impressions). Aggregate the long tail as "Other (N advertisers)" rather than
  list low-volume ones.
