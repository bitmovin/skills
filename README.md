...
- The core skill lives in [`skills/bitmovin-player-web/SKILL.md`](skills/bitmovin-player-web/SKILL.md)
- `.claude-plugin/` provides a Claude Code plugin wrapper
- `plugins/bitmovin-player-web/.codex-plugin/plugin.json` provides a Codex plugin wrapper
...
Verify with *"What skills do you have access to?"* — `bitmovin-player-web` should appear.
...
That gives ChatGPT/Codex the same Bitmovin Player Web guidance without depending on the Claude-specific plugin wrapper.

## Install With skills.sh

This repository is compatible with [`skills.sh`](https://skills.sh) because the canonical skill lives under `skills/bitmovin-player-web/SKILL.md`, which is one of the repository layouts that `npx skills` discovers automatically.
...
npx skills add bitmovin/bitmovin-player-skill --skill bitmovin-player-web

# Install to specific agents
npx skills add bitmovin/bitmovin-player-skill --skill bitmovin-player-web -a claude-code -a codex

# Install globally without prompts
npx skills add bitmovin/bitmovin-player-skill --skill bitmovin-player-web -g -y
```
...
This repo now includes a Codex plugin wrapper at `plugins/bitmovin-player-web/` plus repo-local marketplace metadata at `.agents/plugins/marketplace.json`.
...
If you already have `~/.agents/plugins/marketplace.json`, merge in the `bitmovin-player-web` entry instead of overwriting the file.
...
- `skills/bitmovin-player-web/SKILL.md`: the portable skill content
- `.claude-plugin/plugin.json`: Claude Code plugin metadata
- `.claude-plugin/marketplace.json`: Claude marketplace metadata
- `plugins/bitmovin-player-web/.codex-plugin/plugin.json`: Codex plugin metadata
- `plugins/bitmovin-player-web/skills/bitmovin-player-web/SKILL.md`: symlink to the shared root skill
...
LLMs have stale or confused Bitmovin Player Web knowledge. Common failures without this plugin:
...
The skill file is plain markdown and can also be reused in other agent environments that support local skills, such as Cursor, Copilot, Codex, Goose, Gemini CLI, and Cline. The host-specific wrappers in this repo are `.claude-plugin/` for Claude Code and `plugins/bitmovin-player-web/.codex-plugin/` for Codex.

## Contributing

The source of truth is `skills/bitmovin-player-web/SKILL.md`. Keep it portable across hosts. If you change packaged plugin behavior or published metadata, keep these wrappers in sync:
...
The Codex plugin skill path is intentionally a symlink back to `skills/bitmovin-player-web/SKILL.md` so the repo only has one canonical skill payload.
